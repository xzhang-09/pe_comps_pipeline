...
> **Historical note (superseded).** This records the benchmark's **v1**
> iteration (9 deals, no discovery ladder) — see
> [`known_limitations_and_roadmap.md`](known_limitations_and_roadmap.md#what-this-evaluates)
> for the version scheme. Two later revisions superseded it (v2: 10 deals
> with the discovery ladder and coverage waterfall; v3, current: 16 deals
> with a dev/holdout split). Every number below is superseded; none of it
> reflects the current benchmark. For current methodology, numbers, and
> open items, see
...
This note captures why the first full manual-deals evaluation (v1) produced
`0.0%` Precision@15 and what to improve next. It is based on the run written
to `eval/results.md` and `outputs/eval/manual_deals/results.json`.
